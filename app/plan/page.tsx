...
// Ajusta a tu shape real del /api/plan
...
type PlanResponse = {
  ...
  car: CarBaseline;
  flights: Flight[];
  // Si tu API devuelve también razonamiento/explicación, añádelo aquí
};
...
export default function PlanPage() {
  ...
  const flightsSorted = useMemo(() => {
    ...
    // Si ya vienen ordenados por score, esto no cambia nada; si no, los ordena.
        ...
        <section className="grid gap-6">
          <div className="rounded-2xl p-4 bg-neutral-900 shadow">
            ...
            <div className="grid md:grid-cols-3 gap-3 text-neutral-200">
              <div><span className="text-neutral-400">Distancia</span><div className="text-lg">{data.car.distance_km.toFixed(0)} km</div></div>
              <div><span className="text-neutral-400">Duración</span><div className="text-lg">{minsToHM(data.car.duration_minutes)}</div></div>
              <div><span className="text-neutral-400">Coste estimado</span><div className="text-lg">~ {data.car.cost_eur.toFixed(0)} €</div></div>
            </div>
                ...
                <li key={f.id} className="rounded-2xl p-4 bg-neutral-900 shadow">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-semibold">{f.price_total.toFixed(0)} {f.currency}</div>
